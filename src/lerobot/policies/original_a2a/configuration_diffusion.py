...
@PreTrainedConfig.register_subclass("original_a2a")
@dataclass
class OriginalA2AConfig(PreTrainedConfig):
    n_obs_steps: int = 8
    horizon: int = 16
    ...
    # Keep boundary-centered windows and let LeRobot's dataset querying clamp/pad them,
    # matching the source A2A dataset behavior (`pad_before=n_obs_steps-1`, `pad_after=n_action_steps-1`)
    # instead of dropping tail frames.
    ...
    def __post_init__(self) -> None:
        ...
        # Source A2A keeps start/end windows via padding instead of dropping the trailing
        # frames that don't have a full future horizon available.
        ...
        if self.robot_state_feature is not None and self.robot_state_feature.shape is not None:
            if len(self.robot_state_feature.shape) != 1:
                raise ValueError("`observation.state` must be a 1D feature for original_a2a.")
        ...
        if self.horizon < self.n_obs_steps - 1 + self.n_action_steps:
            raise ValueError(
                "`horizon` must satisfy horizon >= n_obs_steps - 1 + n_action_steps for original_a2a future slicing."
            )
    ...
    def validate_features(self) -> None:
        if self.robot_state_feature is None:
            raise ValueError("original_a2a requires `observation.state` as an input feature.")
        if self.action_feature is None:
            raise ValueError("original_a2a requires `action` as an output feature.")
        if len(self.image_features) > 0:
            first_image_key, first_image_ft = next(iter(self.image_features.items()))
            for key, image_ft in self.image_features.items():
                if image_ft.shape != first_image_ft.shape:
                    raise ValueError(
                        f"`{key}` does not match `{first_image_key}`, but original_a2a expects all image shapes to match."
                    )

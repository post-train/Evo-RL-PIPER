...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _getitem_with_retry(self, idx: int, max_retries: int = 20) -> dict:
        for attempt in range(max_retries + 1):
            candidate_idx = (idx + attempt) % self.num_frames
            try:
                return self._getitem_impl(candidate_idx)
            except AssertionError as err:
                if "unexpectedly violate the tolerance" not in str(err):
                    raise
                logging.warning(
                    "Skipping dataset item %s due to video timestamp mismatch on attempt %s/%s: %s",
                    candidate_idx,
                    attempt + 1,
                    max_retries + 1,
                    err,
        ...
        raise RuntimeError(
            f"Failed to fetch a valid dataset item after {max_retries + 1} attempts starting from index {idx}."
        )

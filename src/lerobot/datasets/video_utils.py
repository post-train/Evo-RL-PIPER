...
def is_torchcodec_available() -> bool:
    """Return whether torchcodec is importable and its runtime libraries can be loaded."""
    ...
    try:
        ...
        logging.warning(
            "torchcodec is installed but unavailable at runtime; falling back to 'pyav'. Error: %s",
            error,
        )
        return False
...
def decode_video_frames(
    ...
    if backend == "torchcodec":
        try:
            ...
            logging.warning(
                "torchcodec decode failed for %s; falling back to 'pyav'. Error: %s",
                video_path,
                error,
            )
...
def decode_video_frames_pyav(
    ...
    """Loads frames with PyAV directly.

    Torchvision 0.26 removed ``torchvision.io.VideoReader``. Keep ``pyav`` as a
    supported backend by decoding frames through PyAV instead of Torchvision.
    """
    video_path = str(video_path)
    query_ts = torch.tensor(timestamps, dtype=torch.float64)
    first_ts = float(query_ts.min())
    last_ts = float(query_ts.max())
    ...
    with av.open(video_path, "r") as container:
        ...
        seek_target = int(seek_ts / float(video_stream.time_base))
        ...
        for frame in container.decode(video=0):
            ...
            current_ts = float(frame.pts * video_stream.time_base)
            if current_ts < seek_ts:
                continue
    ...
    if not loaded_ts:
        raise AssertionError(f"No video frames could be decoded from {video_path}.")
...
def decode_video_frames_torchvision(
    ...
    if not hasattr(torchvision.io, "VideoReader"):
        raise RuntimeError(
            ...
            "Use backend='pyav' or install a torchvision build that provides VideoReader."
        )

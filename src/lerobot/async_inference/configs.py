...
class RobotClientConfig:
    ...
    def __post_init__(self):
        ...
        # ABPolicy-style CAGE execution should continuously refresh with the latest chunk
        # and replace pending actions rather than blending them.
        if self.policy_type in {"abpolicy", "cage"} and self.chunk_size_threshold == 0.5:
            self.chunk_size_threshold = 1.0
        if self.policy_type in {"abpolicy", "cage"} and self.aggregate_fn_name == "weighted_average":
            self.aggregate_fn_name = "latest_only"

...
class OpenCVCamera(Camera):
    ...
    def _validate_fps(self) -> None:
        ...
        if not math.isclose(self.fps, actual_fps, rel_tol=1e-3):
            raise RuntimeError(f"{self} failed to set fps={self.fps} (actual_fps={actual_fps}).")
    ...
    def _validate_width_and_height(self) -> None:
        ...
        if self.capture_width != actual_width:
            raise RuntimeError(
                f"{self} failed to set capture_width={self.capture_width} (actual_width={actual_width})."
            )

        actual_height = int(round(self.videocapture.get(cv2.CAP_PROP_FRAME_HEIGHT)))
        if self.capture_height != actual_height:
            raise RuntimeError(
                f"{self} failed to set capture_height={self.capture_height} (actual_height={actual_height})."
            )

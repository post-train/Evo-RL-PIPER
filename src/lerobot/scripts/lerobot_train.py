...
from lerobot.datasets.sampler import EpisodeAwareSampler, compute_motion_sampling_weights
...
def update_policy(
    ...
    policy.train()
    is_first_step = train_metrics.step == 0
    ...
    with accelerator.autocast():
        ...
        if rabc_batch_weights is not None:
            ...
            if is_first_step:
                logging.info("First training step: entering policy.forward(reduction='none')")
            ...
            if is_first_step:
                logging.info("First training step: entering policy.forward()")
    ...
    if is_first_step:
        logging.info("First training step: forward finished, entering backward()")
    ...
    # Optimizer step
    if is_first_step:
        logging.info("First training step: backward finished, entering optimizer.step()")
    with lock if lock is not None else nullcontext():
        optimizer.step()

    optimizer.zero_grad()
    if is_first_step:
        logging.info("First training step: optimizer.step() finished")
...
def train(
    ...
    """
    if cfg.policy is not None and cfg.policy.type in {"abpolicy", "cage"}:
        cfg.use_policy_training_preset = True
    cfg.validate()
    if cfg.policy.type in {"abpolicy", "cage"}:
        cfg.use_policy_training_preset = True
        logging.info("Using ABPolicy-aligned lerobot_train flow with policy-defined optimizer groups.")
    ...
    if is_main_process:
        logging.info("Accelerator runtime device: %s", device)
        logging.info("torch.cuda.is_available(): %s", torch.cuda.is_available())
        if torch.cuda.is_available():
            logging.info("torch.cuda.device_count(): %d", torch.cuda.device_count())
    ...
    if cfg.dataset.motion_weighted_sampling and not cfg.dataset.streaming:
        shuffle = False
        motion_weights = compute_motion_sampling_weights(
            dataset,
            threshold=cfg.dataset.motion_sampling_threshold,
            static_weight=cfg.dataset.motion_sampling_static_weight,
            max_weight=cfg.dataset.motion_sampling_max_weight,
            drop_n_first_frames=drop_n_first_frames,
            drop_n_last_frames=drop_n_last_frames,
        )
        num_samples = int((motion_weights > 0).sum().item())
        ...
        if is_main_process:
            positive = motion_weights[motion_weights > 0]
            logging.info(
                "Motion-weighted sampling enabled: threshold=%.3f static_weight=%.3f max_weight=%.3f "
                "active_frames=%d mean_weight=%.3f p90_weight=%.3f",
                cfg.dataset.motion_sampling_threshold,
                cfg.dataset.motion_sampling_static_weight,
                cfg.dataset.motion_sampling_max_weight,
                num_samples,
                positive.mean().item(),
                torch.quantile(positive, 0.9).item(),
            )

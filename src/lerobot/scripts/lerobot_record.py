...
Example recording with bimanual piper:
```shell
lerobot-record \
...
class RecordConfig:
    ...
    # Real-Time Chunking controls for chunked flow policies.
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    try:
        ...
        env_force_rtc_async = os.getenv("LEROBOT_RTC_ASYNC_INFERENCE")
        if env_force_rtc_async is not None:
            cfg.rtc.async_inference = env_force_rtc_async.lower() in {"1", "true", "yes", "on"}
        logging.info("RTC config at record entry: %s", cfg.rtc)
        if policy is not None:
            logging.info(
                "Loaded policy path=%s n_obs_steps=%s n_action_steps=%s horizon=%s",
                cfg.policy.pretrained_path if cfg.policy is not None else None,
                getattr(policy.config, "n_obs_steps", None),
                getattr(policy.config, "n_action_steps", None),
                getattr(policy.config, "horizon", None),
            )
        if policy is not None and cfg.rtc.enabled:
            enable_policy_rtc(policy, cfg.rtc)
        preprocessor = None
        postprocessor = None
        ...
        def wait_for_start_key():
            ...
            events["start_episode"] = False
            events["recording_mode"] = "recording"
            return True

        def wait_for_save_or_rerecord():
            """录制完成后等待用户选择保存或重录"""
            logging.info("\n" + "="*50)
            logging.info("录制完成！")
            logging.info("  按 -> 键：保存并继续下一轮")
            logging.info("  按 <- 键：重新录制当前轮")
            logging.info("  按 ESC 键：退出程序")
            logging.info("="*50)
            
            events["save_episode"] = False
            events["rerecord_episode"] = False
            events["recording_mode"] = "finished"
            while not events["save_episode"] and not events["rerecord_episode"] and not events["stop_recording"]:
                time.sleep(0.05)
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                events["recording_mode"] = "recording"
                logging.info("\n" + "="*50)
                logging.info(f"开始录制第 {dataset.num_episodes + 1} 轮")
                logging.info("="*50)
                record_loop(
                    robot=robot,
                    events=events,
                ...
                result = wait_for_save_or_rerecord()
                if result == "stop":
                    break
                elif result == "rerecord":
                ...
                recorded_episodes += 1
                logging.info(f"已保存第 {recorded_episodes} 轮，共需录制 {cfg.dataset.num_episodes} 轮")

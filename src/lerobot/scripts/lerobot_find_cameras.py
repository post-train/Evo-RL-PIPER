...
def create_camera_instance(cam_meta: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        if cam_type == "OpenCV":
            ...
            if "video4" in cam_id_str or cam_id_str.endswith("4"):
        ...
        if instance:
            ...
            # Print actual camera settings after connection
            if cam_type == "OpenCV" and instance.videocapture:
                actual_fps = instance.videocapture.get(cv2.CAP_PROP_FPS)
                actual_width = instance.videocapture.get(cv2.CAP_PROP_FRAME_WIDTH)
                actual_height = instance.videocapture.get(cv2.CAP_PROP_FRAME_HEIGHT)
                actual_fourcc_code = int(instance.videocapture.get(cv2.CAP_PROP_FOURCC))
                actual_fourcc = "".join([chr((actual_fourcc_code >> 8 * i) & 0xFF) for i in range(4)])
                print(f"\nActual camera settings for {cam_id}: {actual_width}x{actual_height} @ {actual_fps}fps, FOURCC={actual_fourcc}")

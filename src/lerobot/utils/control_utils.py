...
class TTYKeyboardListener:
    ...
    def _handle_key(self, key: str):
        ...
        if normalized == "RIGHT":
            if self.events.get("recording_mode") == "waiting":
                print("Right arrow key pressed. Starting recording...")
                self.events["start_episode"] = True
            elif self.events.get("recording_mode") == "finished":
                print("Right arrow key pressed. Saving episode...")
                self.events["save_episode"] = True
                self.events["exit_early"] = True
            else:
                print("Right arrow key pressed. Exiting loop...")
                self.events["exit_early"] = True
        elif normalized == "LEFT":
            if self.events.get("recording_mode") == "finished":
            ...
            if not self.events.get("stop_recording", False):
        ...
        def on_press(key):
            try:
                if key == keyboard.Key.right:
                    if events.get("recording_mode") == "waiting":
                        print("Right arrow key pressed. Starting recording...")
                        events["start_episode"] = True
                    elif events.get("recording_mode") == "finished":
                        print("Right arrow key pressed. Saving episode...")
                    ...
                    if events.get("recording_mode") == "finished":
                        print("Left arrow key pressed. Re-recording episode...")

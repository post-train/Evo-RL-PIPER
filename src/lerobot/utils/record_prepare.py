...
# 目标位置 (单位：度)
# 请根据实际情况修改 CAN0 和 CAN1 的目标角度
TARGET_CAN0 = [-1.37, -2.13, 1.87, -3.34, 22.50, -1.96]
TARGET_CAN1 = [0.00, -2.14, 2.32, 0.00, 18.60, 0.00]
...
def main():
    # 创建配置
    config_can0 = PiperFollowerConfig(
        port="can0",
        require_calibration=False,
        enable_on_connect=True,
        speed_ratio=30,  # 速度限制 30%
    )
    config_can1 = PiperFollowerConfig(
        port="can1",
        require_calibration=False,
        enable_on_connect=True,
        speed_ratio=30,
    ...
    # 创建机械臂实例
    print("创建机械臂实例...")
    arm_can0 = PiperFollower(config=config_can0)
    arm_can1 = PiperFollower(config=config_can1)
    arm_can2 = PiperFollower(config=config_can2)
    arm_can3 = PiperFollower(config=config_can3)
    
    # 连接机械臂
    print("连接 CAN0 机械臂...")
    arm_can0.connect(calibrate=False)
    print("连接 CAN1 机械臂...")
    arm_can1.connect(calibrate=False)
    print("连接 CAN2 机械臂...")
    ...
    # 读取当前位置
    obs_can0 = arm_can0.get_observation()
    obs_can1 = arm_can1.get_observation()
    obs_can2 = arm_can2.get_observation()
    obs_can3 = arm_can3.get_observation()
    
    print("\n当前位置:")
    print(f"CAN0: {[obs_can0.get(f'joint_{i}.pos', 0) for i in range(1, 7)]}")
    print(f"CAN1: {[obs_can1.get(f'joint_{i}.pos', 0) for i in range(1, 7)]}")
    ...
    print("\n目标位置:")
    print(f"CAN0: {TARGET_CAN0}")
    print(f"CAN1: {TARGET_CAN1}")
    ...
    cur_pos_can0 = [obs_can0.get(f'joint_{i}.pos', 0) for i in range(1, 7)]
    cur_pos_can1 = [obs_can1.get(f'joint_{i}.pos', 0) for i in range(1, 7)]
    ...
    for i in range(1, steps + 1):
        ...
        # 计算插补位置
        step_pos_can0 = [cur + (tar - cur) * ratio for cur, tar in zip(cur_pos_can0, TARGET_CAN0)]
        step_pos_can1 = [cur + (tar - cur) * ratio for cur, tar in zip(cur_pos_can1, TARGET_CAN1)]
        ...
        # 发送动作
        arm_can0.send_action(deg_to_action(step_pos_can0))
        arm_can1.send_action(deg_to_action(step_pos_can1))
    ...
    # 读取最终位置
    final_obs_can0 = arm_can0.get_observation()
    final_obs_can1 = arm_can1.get_observation()
    final_obs_can2 = arm_can2.get_observation()
    final_obs_can3 = arm_can3.get_observation()
    
    print("\n最终位置:")
    print(f"CAN0: {[final_obs_can0.get(f'joint_{i}.pos', 0) for i in range(1, 7)]}")
    print(f"CAN1: {[final_obs_can1.get(f'joint_{i}.pos', 0) for i in range(1, 7)]}")

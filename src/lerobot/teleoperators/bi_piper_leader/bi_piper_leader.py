...
class _PiperLeaderProcessProxy:
    ...
    def _call(self, command: str, *args, **kwargs):
        ...
        try:
            ...
            raise RuntimeError(f"bi_piper leader worker command '{command}' interrupted") from exc
        if response["ok"]:
...
class BiPiperLeader(Teleoperator):
    ...
    def __init__(self, config: BiPiperLeaderConfig | BiPiperXLeaderConfig):
        ...
        self._action_stop_event: threading.Event | None = None
        self._action_ready_event = threading.Event()
    ...
    def _collect_action(self) -> RobotAction:
    ...
    def _action_loop(self) -> None:
        ...
        while not stop_event.is_set():
            try:
                action_dict = self._collect_action()
                with self._action_lock:
                    self._latest_action = action_dict
                self._action_ready_event.set()
            except (EOFError, BrokenPipeError, KeyboardInterrupt, OSError):
                break
            except Exception:
                if stop_event.is_set() or not self.is_connected:
                    break
                logger.exception("BiPiperLeader action refresh failed.")
                time.sleep(0.005)
    ...
    def _start_action_thread(self) -> None:
        ...
        self._action_thread = threading.Thread(
            ...
            name=f"{self.name}_action_loop",
            daemon=True,
        )
        self._action_thread.start()

    def _stop_action_thread(self) -> None:
        if self._action_stop_event is not None:
        ...
        self._action_stop_event = None
        self._action_ready_event.clear()

    @check_if_not_connected
    def get_action(self) -> RobotAction:
        if not self._action_ready_event.wait(timeout=1.0):
            logger.warning("BiPiperLeader action snapshot not ready; falling back to synchronous read.")

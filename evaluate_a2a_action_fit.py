...
import lerobot.policies.a2a.configuration_diffusion  # noqa: F401
import lerobot.policies.original_a2a.configuration_diffusion  # noqa: F401
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate A2A action fitting, with emphasis on episode starts.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional cap on number of frame_index==0 windows to evaluate.",
    )
    parser.add_argument(
        "--max-early-windows",
        type=int,
        default=None,
        help="Optional cap on number of early windows to evaluate.",
    )
    parser.add_argument(
        "--max-overall-windows",
        type=int,
        default=256,
        help="Number of randomly sampled windows for overall fit statistics.",
    )
    parser.add_argument(
        "--early-frame-threshold",
        type=int,
        default=8,
        help="Treat frame_index < threshold as early windows.",
    )
...
def to_batched_policy_input(
    ...
    batch = {
        OBS_STATE: proc[OBS_STATE].unsqueeze(0),
        OBS_IMAGES: torch.stack([proc[key].unsqueeze(0) for key in image_features], dim=-4),
    }
    if include_history_actions:
        batch[ACTION] = proc[ACTION][:n_obs_steps].unsqueeze(0)
    return batch


def generate_actions(policy, batch: dict[str, torch.Tensor]) -> torch.Tensor:
    if hasattr(policy, "model"):
        return policy.model.generate_actions(batch)
    raise TypeError(f"Unsupported policy object for A2A evaluation: {type(policy).__name__}")
...
def summarize_split(
    ...
    for idx in indices:
        raw = dataset[idx]
        batch = to_batched_policy_input(
            raw,
            preprocessor,
            image_features,
            n_obs_steps,
            include_history_actions=policy_type == "original_a2a",
        )
    ...
    def keyed(values):
        return {name: float(value) for name, value in zip(action_names, values)}
...
def main() -> None:
    ...
    output_path = args.output or checkpoint.parent.parent / "a2a_action_fit_report.json"
    ...
    report = {
        ...
        "action_prediction_mode": getattr(cfg.policy, "action_prediction_mode", "absolute"),
    ...
    amp_ratio = report["start_windows"]["delta_abs_amplitude_ratio_pred_over_gt"]
    for joint in focus_joints:
        if joint in amp_ratio:
            summary[f"{joint}_start_amp_ratio"] = amp_ratio[joint]

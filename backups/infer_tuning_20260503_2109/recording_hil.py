...
class RTCPolicyRuntime:
    ...
    prev_returned_action: torch.Tensor | None = None
    last_action_delta: torch.Tensor | None = None
    underrun_streak: int = 0
    ...
    def reset(self) -> None:
        ...
        self.prev_returned_action = None
...
def _compute_async_prefetch_threshold(
    ...
    *,
    ...
    observed_delay = max(inference_delay, runtime.last_real_delay)
    # Keep more queue headroom than the bare observed delay so chunk handoff
    # happens with margin instead of repeatedly dropping to 2-3 actions left.
    proactive_threshold = min(chunk_steps - 1, observed_delay * 2 + 1)
    dynamic_threshold = max(
        runtime.cfg.execution_horizon,
        observed_delay + runtime.cfg.async_prefetch_margin_steps,
        proactive_threshold,
    )
    if runtime.chunk_counter <= 2:
        dynamic_threshold = chunk_steps - 1
    ...
    *,
    ...
    def _worker() -> None:
        ...
        while not runtime.async_stop_event.is_set():
            ...
            original_actions, processed_actions, inference_latency = _run_rtc_inference_from_history_snapshot(
                runtime=runtime,
                policy=policy,
                postprocessor=postprocessor,
...
def _run_rtc_inference_from_history_snapshot(
    *,
    runtime: RTCPolicyRuntime,
    policy: PreTrainedPolicy,
    postprocessor: PolicyProcessorPipeline[PolicyAction, PolicyAction],
    ...
    with (
        ...
        with runtime.inference_lock:
            prepared_observation = _restore_policy_history_snapshot(policy, history_snapshot)
            action_chunk = policy.predict_action_chunk(
                prepared_observation,
                inference_delay=inference_delay if use_rtc_guidance else 0,
                prev_chunk_left_over=prev_actions if use_rtc_guidance else None,
    ...
    return original_actions, processed_actions, inference_latency
...
def _merge_rtc_inference_result(runtime: RTCPolicyRuntime, result: RTCInferenceResult) -> None:
    ...
    if result.is_bootstrap_chunk:
        ...
        bootstrap_blend_steps = min(
            len(result.processed_actions),
            runtime.cfg.async_preserve_prefix_steps + runtime.cfg.async_transition_blend_steps,
        )
        if bootstrap_blend_steps > 0 and runtime.last_returned_action is not None:
            hold_action_processed = runtime.last_returned_action.to(
                device=runtime.action_queue.queue.device,
                dtype=runtime.action_queue.queue.dtype,
            ).unsqueeze(0)
            processed_blend_weights = torch.linspace(
                1.0 / (bootstrap_blend_steps + 1),
                bootstrap_blend_steps / (bootstrap_blend_steps + 1),
                steps=bootstrap_blend_steps,
                device=runtime.action_queue.queue.device,
                dtype=runtime.action_queue.queue.dtype,
            ).unsqueeze(-1)
            runtime.action_queue.queue[:bootstrap_blend_steps] = (
                (1.0 - processed_blend_weights) * hold_action_processed
                + processed_blend_weights * runtime.action_queue.queue[:bootstrap_blend_steps]
            )

            hold_action_original = runtime.last_returned_action.to(
                device=runtime.action_queue.original_queue.device,
                dtype=runtime.action_queue.original_queue.dtype,
            ).unsqueeze(0)
            ...
            runtime.action_queue.original_queue[:bootstrap_blend_steps] = (
                (1.0 - original_blend_weights) * hold_action_original
                + original_blend_weights * runtime.action_queue.original_queue[:bootstrap_blend_steps]
            )
            logger.info(
                "RTC bootstrap transition smoothing applied: steps=%d",
                bootstrap_blend_steps,
            )
        ...
        if not result.use_rtc_guidance:
            ...
            preserve_prefix_steps = runtime.cfg.async_preserve_prefix_steps
            transition_blend_steps = runtime.cfg.async_transition_blend_steps
            if result.queue_size_before <= 5:
                preserve_prefix_steps = max(preserve_prefix_steps, 4)
                transition_blend_steps = max(transition_blend_steps, 4)
            elif result.queue_size_before <= 6:
                preserve_prefix_steps = max(preserve_prefix_steps, 3)
                transition_blend_steps = max(transition_blend_steps, 3)
    ...
    logger.info(
        "RTC chunk=%d q_before=%d q_after=%d action_idx=%d prev_left=%d inf_delay=%d real_delay=%d latency=%.2fms max_latency=%.2fms exec_h=%d chunk_steps=%d guidance=%s preserve=%d blend=%d",
        ...
        preserve_prefix_steps,
        transition_blend_steps,
    )


def _record_returned_action(runtime: RTCPolicyRuntime, action: torch.Tensor, *, reset_underrun: bool) -> None:
    if runtime.last_returned_action is not None:
        last_action = runtime.last_returned_action.to(device=action.device, dtype=action.dtype)
        runtime.prev_returned_action = last_action.clone()
        runtime.last_action_delta = action - last_action
    runtime.last_returned_action = action.clone()
    if reset_underrun:
        runtime.underrun_streak = 0


def _build_underrun_fallback_action(runtime: RTCPolicyRuntime) -> torch.Tensor | None:
    if runtime.last_returned_action is None:
        return None
    if runtime.last_action_delta is None:
        return runtime.last_returned_action.clone()

    last_action_delta = runtime.last_action_delta.to(
        device=runtime.last_returned_action.device,
        dtype=runtime.last_returned_action.dtype,
    )
    decay = 0.5 ** min(runtime.underrun_streak + 1, 3)
    fallback_delta = last_action_delta * decay
    fallback_action = runtime.last_returned_action + fallback_delta
    runtime.last_returned_action = fallback_action.clone()
    runtime.underrun_streak += 1
...
def predict_action_with_rtc(
    *,
    ...
    _record_returned_action(runtime, action, reset_underrun=True)
    return action
...
def predict_action_with_rtc_async(
    *,
    ...
    if runtime.last_returned_action is not None:
        fallback_action = _build_underrun_fallback_action(runtime)
        logger.warning(
            "RTC async queue underrun; extrapolating the recent action trend while waiting for request=%s streak=%d",
            runtime.pending_request_id,
            runtime.underrun_streak,
        )
